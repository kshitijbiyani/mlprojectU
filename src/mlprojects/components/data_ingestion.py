...
class DataIngestion:
    ...
    def initiate_data_ingestion(self):
        ...
        try:
            df_Main=pd.read_csv("notebook\data\stud.csv")
            logging.info("Data Read Completed")
            os.makedirs(os.path.dirname(self.ingestion_config.train_data_path),exist_ok=True)
            df_Main.to_csv(self.ingestion_config.raw_data_path,index=False, header=True)
            logging.info("Train Test Split Initiated")
            train_set,test_set=train_test_split(df_Main,test_size=0.2, random_state=42)
